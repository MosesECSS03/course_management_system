...
class CreateAccountsSection extends Component {
  constructor(props) {
    super(props);
    this.state = {
        roleOptions: ['Ops in-charge', 'NSA in-charge', 'Site in-charge', 'Finance'],
        filteredRoles: [],
        site: ""
    };
  }  
  ...
  handleSubmit = (event) => {
   ...
   const { name, email, password, role, site } = this.state;
    ...
    else if(role !== "Ops in-charge" && role !== "NSA in-charge" && role !== "Site in-charge" && role !== "Finance")
    {
      roleError = 'Role must be Ops in-charge, NSA in-charge, Site in-charge or Finance';
      valid = false;
    }
    ...
    else 
    {
        ...
        axios.post('https://moses-ecss-course.azurewebsites.net/accountDetails', {"accountDetails": accountDetails, "purpose": "create"})
