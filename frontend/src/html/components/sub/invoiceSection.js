...
class InvoiceSection extends Component {
    ...
    fetchInvoiceDetails = async () => {
        try {
            ...
            const response = await axios.post('https://moses-ecss-data.azurewebsites.net/invoice_report/');
    ...
    generateInvoice = async () => 
    {
        ...
        if (courses.length > 0 || months.length > 0) 
        {
            try 
            {
                ...
                var response = await axios.post('https://moses-ecss-backend.azurewebsites.net/invoice', {purpose: 'findInvoiceNumber', selectedMonth: selectedMonth});
                ...
                if(invoiceNumber === "")
                {
                    ...
                    var response1 = await axios.post(' https://moses-ecss-backend.azurewebsites.net/invoice', {purpose: 'getInvoiceNumber'});
                    ...
                    var response2 = await axios.post('https://moses-ecss-backend.azurewebsites.net/invoice', { purpose: 'generateInvoice', details: courses, totalPrice: totalPrice, totalPriceInWords: totalPriceInWords, invoiceNumber: newInvoiceNumber, selectedMonth: selectedMonth, userName: this.props.userName}, { responseType: 'blob' });
                ...
                else
                {
                    ...
                    var response1 = await axios.post('https://moses-ecss-backend.azurewebsites.net/invoice', { purpose: 'generateInvoice', details: courses, totalPrice: totalPrice, totalPriceInWords: totalPriceInWords, invoiceNumber: invoiceNumber, selectedMonth: selectedMonth, userName: this.props.userName}, { responseType: 'blob' })
                    // Extract filename from Content-Disposition header
